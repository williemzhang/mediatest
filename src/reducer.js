    ...
    case BUTTON_PRESS:
      ...
      if (action.payload == "C") {
            ...
            switch (pressedArr[lastIndex - 1]) {
              case "/":
                var newVal = parseFloat(
                  pressedArr[lastIndex - 2] / pressedArr[lastIndex]
                );
          ...
          for (var i = 0; i < pressedArr.length; i++) {
            if (i == 0) {
              ...
              switch (pressedArr[i]) {
                case "-":
                  lastValue =
                    parseFloat(lastValue) - parseFloat(pressedArr[i + 1]);
        ...
        if (
          ...
          operatorRegex.test(pressedArr[lastIndex]) &&
          pressedArr.length > 0
        ) {
          pressedArr[lastIndex] = action.payload;
        } else if (
          numberRegex.test(action.payload) &&
          numberRegex.test(pressedArr[lastIndex]) &&
          pressedArr.length > 0
        ) {
          if (pressedArr.length > 0) {
            if (action.payload == ".") {
